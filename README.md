...
Кратко: **план закупки** (wizard 5 шагов), словарь + спецификации, авто-восстановление SKU при загрузке прайса, настройки локаций, `start-local.cmd`.  
Деплой на VPS: [`docs/deploy-vps.md`](docs/deploy-vps.md).

Дополнительно в актуальной версии:

- Улучшен парсер спроса на шаге `1. Спрос`:
  - понимает строки без единицы (`Картофель 7` -> `7 кг`)
  - понимает слитные формы (`5кг`)
  - чистит разделитель перед количеством (`редис - 4кг`)
  - понимает `N пачек по X` (`Руккола 5 пачек по 0125` -> `0.625 кг`)
  - поддерживает записи с ведущим нулем как десятичные (`03` -> `0.3 кг`, `025` -> `0.25 кг`)
- На шаге `4. Заказы` добавлен блок **«Сообщения поставщикам»**:
  - отдельный текст по каждому поставщику
  - формат `Локация + Отдел` -> строки `товар, количество, спецификация`
  - кнопка копирования текста для отправки поставщику
- На шаге `3. Распределение` доступны режимы:
  - `optimize_min_order` (дефолт, с учетом минимальных заказов),
  - `cheapest_only` (чистая минимальная цена по позициям),
  - `hybrid_topup` (минимальная цена + подсказки добора до минимального заказа).
...
## 1.1) План закупки: ввод спроса (новые форматы)

`backend/app/parsers/order_parser.py` поддерживает:

- `Название 3 кг`
- `Название (3 кг)`
- `Название 3` (по умолчанию кг)
- `Название - 3кг`
- `Название 5 пачек по 0125`

Для записей с ведущим нулем без запятой:
...
Важно: наличие `ai_available=true` в ответе `/match` означает, что ключи есть, но не гарантирует назначений.  
Факт участия ИИ в конкретном запуске смотрите по `match_mode` и `ai_assigned_count`.
