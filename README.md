...
Ветка: `fix/match-resilience-and-ux` (коммит `6d1e361` и новее).

- **Прайсы:** рядом с «Загружено позиций» показывается дата последней загрузки файла; после загрузки — зелёная плашка «только что загружен».
- **Заказ:** текст заказа и результат «Проверить ввод» не пропадают при переходе на «Результат» и обратно; кнопка **Очистить** для нового списка.
- **Заказ (парсер):** `г` / `гр` → `кг` (÷1000), `л` → `мл` (×1000) — в превью колонка «Пересчёт».
- **Результат:** кнопка **Скачать xlsx** перенесена наверх страницы.
- **Локальный запуск:** скрипты `start-local.cmd` / `start-local.ps1` в корне проекта.
- **Backend:** поле `last_price_upload_at` у поставщика (миграция при старте backend).
